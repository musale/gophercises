...
// URLData - showing a redirect path and a url to redirect to.
type URLData struct {
...
func YAMLHandler(yml []byte, fallback http.Handler) (http.HandlerFunc, error) {
	...
	pathsToUrls := make(map[string]string, len(yamlData))
	for _, path := range yamlData {
		pathsToUrls[path.url] = path.path
	}
